...
pub(super) fn setup_container_mounts(rootless: bool) -> Result<()> {
    ...
    if !rootless { let _ = mount_fs("/sys", "sysfs", "sysfs", noexec | MsFlags::MS_RDONLY, None::<&str>, "failed to mount /sys"); }
    if Path::new("/tmp").is_dir() {
        let _ = mount(Some("tmpfs"), "/tmp", Some("tmpfs"), MsFlags::MS_NOSUID | MsFlags::MS_NODEV, Some("mode=1777"));
    }
    Ok(())
}
...
pub(super) fn prepare_host_device_sources(rootfs: &Path) -> Result<()> {
    ...
    for &name in DEVICES {
        ...
        File::create(&src).with_context(|| format!("failed to create device placeholder at {}", src.display()))?;
        let host = format!("/dev/{name}");
        mount(Some(host.as_str()), src.as_path(), None::<&str>, MsFlags::MS_BIND, None::<&str>)
...
pub(super) fn cleanup_host_device_sources(rootfs: &Path) -> Result<()> {
    let dir = rootfs.join(".hippobox-dev");
    for &name in DEVICES { let _ = umount2(&dir.join(name), MntFlags::MNT_DETACH); }
...
pub(super) fn mask_proc_paths() -> Result<()> {
    ...
    for path in ["/proc/acpi", "/proc/kcore", "/proc/keys", "/proc/latency_stats",
        ...
        } else { mount(Some("/dev/null"), path, None::<&str>, MsFlags::MS_BIND, None::<&str>) };
        if let Err(e) = result { eprintln!("warning: failed to mask {path}: {e}"); continue; }
        if p.is_file() { let _ = mount(None::<&str>, path, None::<&str>, ro, None::<&str>); }
    }
    for path in ["/proc/asound", "/proc/bus", "/proc/fs", "/proc/irq", "/proc/sys", "/proc/sysrq-trigger"] {
...
pub(super) fn mount_fs(
    target: &str, source: &str, fstype: &str, flags: MsFlags, options: Option<&str>, context: &'static str,
) -> Result<()> {
...
pub(super) fn mount_overlay(
    ...
    if rootless && mount(Some("overlay"), merged, Some("overlay"), MsFlags::empty(), Some(format!("{opts},redirect_dir=on").as_str())).is_ok() { return Ok(()); }
...
pub fn parse_volume(spec: &str) -> Result<super::VolumeMount> {
    ...
    let (source, target, read_only) = match parts.len() {
        ...
        3 => match parts[2] {
            "ro" => (parts[0], parts[1], true), "rw" => (parts[0], parts[1], false),
            opt => bail!("invalid volume option {opt:?}, expected 'ro' or 'rw'"),
        },
        _ => bail!("invalid volume spec {spec:?}, expected SRC:DST[:ro|rw]"),
    };
    if source.is_empty() || target.is_empty() { bail!("invalid volume spec {spec:?}, empty source or target"); }
    if !source.starts_with('/') { bail!("volume source must be absolute: {source:?}"); }
    if !target.starts_with('/') { bail!("volume target must be absolute: {target:?}"); }
    if Path::new(target).components().any(|c| matches!(c, std::path::Component::ParentDir)) {
        bail!("volume target must not contain '..': {target:?}");
    }
    if !Path::new(source).exists() { bail!("volume source does not exist: {source:?}"); }
    Ok(super::VolumeMount {
        source: Path::new(source).canonicalize()
            .with_context(|| format!("failed to resolve volume source {source:?}"))?.to_string_lossy().to_string(),
        target: target.to_string(), read_only,
    })
}
...
pub(super) fn mount_volumes(merged: &Path, volumes: &[super::VolumeMount]) -> Result<()> {
    for vol in volumes {
        ...
        if !target.starts_with(merged) { bail!("volume target escapes container rootfs: {}", vol.target); }
        if vol.source == "tmpfs" {
            fs::create_dir_all(&target)?;
            mount(Some("tmpfs"), &target, Some("tmpfs"), MsFlags::MS_NOSUID | MsFlags::MS_NODEV, Some("size=67108864"))
                .with_context(|| format!("failed to mount tmpfs volume at {}", target.display()))?;
            continue;
        }
        let meta = fs::metadata(&vol.source).with_context(|| format!("volume source inaccessible: {}", vol.source))?;
        if meta.is_file() {
            if let Some(parent) = target.parent() { fs::create_dir_all(parent)?; }
            if !target.exists() { File::create(&target)?; }
        } else { fs::create_dir_all(&target)?; }
        mount(Some(vol.source.as_str()), &target, None::<&str>, MsFlags::MS_BIND | MsFlags::MS_REC, None::<&str>)
            .with_context(|| format!("failed to bind-mount {} at {}", vol.source, target.display()))?;
        let mut flags = MsFlags::MS_BIND | MsFlags::MS_REC | MsFlags::MS_REMOUNT | MsFlags::MS_NOSUID | MsFlags::MS_NODEV;
        if vol.read_only { flags |= MsFlags::MS_RDONLY; }
