...
use nix::{sys::{signal::{self, Signal}, wait::{WaitStatus, waitpid}}, unistd::{self, ForkResult, Pid}};
use std::{ffi::CString, os::{fd::{AsRawFd, FromRawFd, IntoRawFd}, unix::process::CommandExt}};

pub(super) fn run_container(mut config: ChildConfig, stop_signal: &str) -> Result<i32> {
    if !config.rootless { super::cleanup::check_cgroup_v2()?; super::cleanup::cgroup_create(&config.container_id)?; }
    let has_ports = !config.port_mappings.is_empty();
    let (ready_read, ready_write) = if has_ports { let (r, w) = nix::unistd::pipe().context("failed to create ready pipe")?; (Some(r), Some(w)) }
        else { (None, None) };
    ...
    let fork_result = match unsafe { unistd::fork() } {
        Ok(r) => r, Err(e) => { let _ = nix::unistd::close(write_raw); return Err(e).context("fork failed"); }
    };

    match fork_result {
        ForkResult::Parent { child } => {
            drop(read_fd); drop(ready_write);
            serde_json::to_writer(std::io::BufWriter::new(unsafe { std::fs::File::from_raw_fd(write_raw) }), &config)?;
            ...
            let stop_signal = match stop_signal.trim_start_matches("SIG") {
                ...
                "USR1" => Signal::SIGUSR1, "USR2" => Signal::SIGUSR2, "KILL" => Signal::SIGKILL, _ => Signal::SIGTERM,
            };
            let exit_code = parent_wait(child, || { let _ = signal::kill(child, stop_signal); })?;
            if let Some(mut pasta) = pasta_child { let _ = pasta.kill(); let _ = pasta.wait(); }
            Ok(exit_code)
        }
        ForkResult::Child => {
            nix::unistd::close(write_raw).context("failed to close config pipe write end")?;
            drop(ready_read); if let Some(rw) = ready_write { let _ = rw.into_raw_fd(); }
            super::set_pdeathsig_with_race_check()?;
            let exe = super::resolve_self_exe()?;
            let exe_c = CString::new(exe.to_string_lossy().as_bytes()).context("current executable path contains NUL")?;
            let err = nix::unistd::execv(&exe_c, &[exe_c.clone(), CString::new("--container-init")?, CString::new(read_raw.to_string())?]).unwrap_err();
            eprintln!("execv failed: {err}"); std::process::exit(127)
        }
    }
}
...
pub(crate) fn parent_wait(child: Pid, forward: impl Fn()) -> Result<i32> {
    ...
    loop {
        match waitpid(child, None) {
            ...
            Err(nix::errno::Errno::EINTR) => { if PENDING_SIGNAL.swap(0, std::sync::atomic::Ordering::SeqCst) != 0 { forward(); } }
...
pub(super) fn to_cstrings(values: &[String], label: &str) -> Result<Vec<CString>> {
    values.iter().map(|v| CString::new(v.as_str())
        .with_context(|| format!("{label} contains NUL byte: {v:?}"))).collect()
}
...
pub(super) fn run_rootless_unshare(spec: super::ContainerSpec) -> Result<i32> {
    ...
    let pre_exec_fn = move || unsafe {
        if nix::libc::setpgid(0, 0) != 0 { return Err(std::io::Error::last_os_error()); }
        super::set_pdeathsig()?;
    ...
    serde_json::to_writer(
        std::io::BufWriter::new(unsafe { std::fs::File::from_raw_fd(spec_write.into_raw_fd()) }), &spec,
    ).context("failed to send rootless bootstrap spec")?;

    let child_pid = Pid::from_raw(child.id() as i32);
    std::mem::forget(child);
    parent_wait(child_pid, || { let _ = unsafe { nix::libc::kill(-child_pid.as_raw(), nix::libc::SIGTERM) }; })
...
mod tests {
    ...
    fn to_cstrings_all_cases() {
        ...
        let inputs: Vec<String> = vec!["/usr/local/bin/my-tool".into(), "hello world".into(),
            "caf\u{00e9}".into(), "--flag=value".into()];
    ...
    fn child_config_serialisation_round_trip() {
        let config = ChildConfig {
            rootfs: "/merged".into(), argv: vec!["/bin/sh".into(), "-c".into(), "echo hello".into()],
