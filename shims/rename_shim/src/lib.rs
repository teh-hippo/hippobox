...
use libc::{c_char, c_void, closedir, lstat, mkdir, mode_t, readdir, readlink, rmdir,
...
#[panic_handler]
fn panic(_: &core::panic::PanicInfo) -> ! { unsafe { libc::abort() } }
...
pub unsafe extern "C" fn rename(old: *const c_char, new: *const c_char) -> c_int {
    ...
    if ret == -1 && unsafe { *libc::__errno_location() } == EXDEV && is_dir_path(old) { return exdev_fallback(old, new); }
    ret
}
...
pub unsafe extern "C" fn renameat(olddirfd: c_int, old: *const c_char, newdirfd: c_int, new: *const c_char) -> c_int {
    ...
    if ret == -1 && unsafe { *libc::__errno_location() } == EXDEV
        && olddirfd == AT_FDCWD && newdirfd == AT_FDCWD && is_dir_path(old) { return exdev_fallback(old, new); }
    ret
}
...
pub unsafe extern "C" fn renameat2(
    ...
    if ret == -1 && unsafe { *libc::__errno_location() } == EXDEV
        && flags == 0 && olddirfd == AT_FDCWD && newdirfd == AT_FDCWD && is_dir_path(old) { return exdev_fallback(old, new); }
    ret
}
...
fn fail_restoring_errno(cleanup: impl FnOnce()) -> c_int {
...
fn exdev_fallback(src: *const c_char, dst: *const c_char) -> c_int {
    ...
    if i == 0 { unsafe { *libc::__errno_location() = libc::ENAMETOOLONG }; return -1; }
    let mut i = i;
    for &b in b".~hb~" { staging[i] = b; i += 1; }
    staging[i] = 0;
    let sp = staging.as_ptr().cast();
    if copy_tree(src, sp) != 0 { return fail_restoring_errno(|| { remove_tree(sp); }); }
    if remove_tree(src) != 0 { return fail_restoring_errno(|| { remove_tree(sp); }); }
    let real: RenameFn = unsafe { resolve(&REAL_RENAME, b"rename\0") };
    if unsafe { real(sp, dst) } != 0 { return fail_restoring_errno(|| { let _ = unsafe { real(sp, src) }; }); }
    0
}
...
fn for_each_child(dir: *const c_char, f: &mut dyn FnMut(*const c_char, *const c_char) -> c_int) -> c_int {
    let fd = unsafe { libc::open(dir, libc::O_RDONLY | libc::O_DIRECTORY | libc::O_NOFOLLOW) };
    if fd < 0 { return -1; }
    let dp = unsafe { libc::fdopendir(fd) };
    if dp.is_null() { unsafe { libc::close(fd) }; return -1; }
    let mut ret = 0;
    loop {
        ...
        if name[0] == b'.' as i8 && (name[1] == 0 || (name[1] == b'.' as i8 && name[2] == 0)) { continue; }
        if f(dir, name.as_ptr()) != 0 { ret = -1; break; }
    }
    unsafe { closedir(dp) };
    ret
}
...
fn copy_dir(src: *const c_char, dst: *const c_char, mode: mode_t) -> c_int {
    if unsafe { mkdir(dst, mode & 0o7777) } != 0 && unsafe { *libc::__errno_location() } != libc::EEXIST { return -1; }
    for_each_child(src, &mut |parent, name| {
        let (mut cs, mut cd) = ([0u8; 4096], [0u8; 4096]);
        if !join_path(parent, name, &mut cs) || !join_path(dst, name, &mut cd) {
            unsafe { *libc::__errno_location() = libc::ENAMETOOLONG }; return -1;
        }
        copy_tree(cs.as_ptr().cast(), cd.as_ptr().cast())
    })
}
...
fn copy_file(src: *const c_char, dst: *const c_char, mode: mode_t) -> c_int {
    ...
    if fd_in < 0 { return -1; }
    let fd_out = unsafe { libc::open(dst, libc::O_WRONLY | libc::O_CREAT | libc::O_TRUNC | libc::O_NOFOLLOW, mode & 0o7777) };
...
fn remove_tree(path: *const c_char) -> c_int {
    ...
    let ret = for_each_child(path, &mut |parent, name| {
        ...
        if !join_path(parent, name, &mut child) { unsafe { *libc::__errno_location() = libc::ENAMETOOLONG }; return -1; }
...
fn copy_cstr(src: *const c_char, buf: &mut [u8; 4096], mut i: usize, limit: usize) -> usize {
    ...
    while unsafe { *p } != 0 {
        if i >= limit { return 0; }
        buf[i] = unsafe { *p }; i += 1; p = unsafe { p.add(1) };
    }
    i
}

fn join_path(dir: *const c_char, name: *const c_char, buf: &mut [u8; 4096]) -> bool {
    let mut i = copy_cstr(dir, buf, 0, 4094);
    if i == 0 { return false; }
