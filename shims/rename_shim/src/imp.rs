...
use libc::{
    AT_FDCWD, EXDEV, S_IFDIR, S_IFLNK, S_IFMT, c_char, c_void, closedir, lstat, mkdir, mode_t,
    readdir, readlink, rmdir, stat as stat_t, symlink, unlink,
};
...
fn exdev_fallback(src: *const c_char, dst: *const c_char) -> c_int {
    ...
    if copy_tree(src, sp) != 0 {
        return fail_restoring_errno(|| {
            remove_tree(sp);
        });
    }
    ...
    let real: RenameFn = unsafe { resolve(&REAL_RENAME, b"rename\0") };
    if unsafe { real(sp, dst) } != 0 {
        return fail_restoring_errno(|| {
            let _ = unsafe { real(sp, src) };
        });
    }
    0
}
...
fn copy_dir(src: *const c_char, dst: *const c_char, mode: mode_t) -> c_int {
    if unsafe { mkdir(dst, mode & 0o7777) } != 0
        && unsafe { *libc::__errno_location() } != libc::EEXIST
    {
        return -1;
    }
